...
# 客服通话数据重采样,多线程
def audio_change_multi(raw_data_dict,new_data_path,sr=16000,channels=1):
    os.makedirs(new_data_path,exist_ok=True)
    
    items = [[key,raw_data_dict[key],new_data_path,sr,channels] for key in raw_data_dict.keys()]
    print(items[0])
    pool = ThreadPool()
    pool.map(audio_change_base, items)
    pool.close()
    pool.join()


# vad
def audio_change_base(item):
    spk,wav_file_paths,new_data_path,sr,channels = item
    
    for wav_file_path in wav_file_paths:
        filename = wav_file_path.split("/")[-1]
        os.makedirs(os.path.join(new_data_path,f"{spk}"),exist_ok=True)
        wav_new_path = os.path.join(new_data_path,f"{spk}/{filename}")
        if os.path.exists(wav_new_path):
            continue
        else:
            cmd = f'ffmpeg -i {wav_file_path} -ac {channels} -ar {sr} {wav_new_path}'
            subprocess.call(cmd, shell=True)
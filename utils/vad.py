...
def vad_kfth_multi(raw_data_dict,new_data_path):
    os.makedirs(new_data_path,exist_ok=True)
    print("Loading...")
    items = [[key,raw_data_dict[key],new_data_path] for key in raw_data_dict.keys()]
...
def read_vad_save(item):
    try:
        ...
        model, utils = torch.hub.load(repo_or_dir='snakers4/silero-vad',
                                ...
                                force_reload=False,
                                onnx=USE_ONNX)
        spk,wav_file_paths,new_data_path = item
        ...
        for wav_file_path in wav_file_paths:
            ...
            if os.path.exists(save_path):
                ...
                try:
                    ...
                    print(f"**Error Pass:{wav_file_path}")
                    continue
                speech_timestamps = get_speech_timestamps(wav, model, sampling_rate=sr,window_size_samples=512)
                
                save_audio(save_path,collect_chunks(speech_timestamps, wav), sampling_rate=sr)
